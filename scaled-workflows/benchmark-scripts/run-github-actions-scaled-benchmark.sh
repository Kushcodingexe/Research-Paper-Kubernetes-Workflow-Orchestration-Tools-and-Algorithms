#!/bin/bash
#===============================================================================
# SCALED (2x) GITHUB ACTIONS BENCHMARK RUNNER - FIXED
...
get_latest_run_for_workflow() {
    # Get the latest run specifically for our workflow file
    curl -s \
        -H "Accept: application/vnd.github+json" \
        -H "Authorization: Bearer ${GITHUB_TOKEN}" \
...
for i in $(seq 1 ${N_RUNS}); do
    ...
    # Wait for the run to appear in the API (increased wait time)
    echo "Waiting for workflow to be queued..."
    sleep 20
    
    # Get the workflow run - use workflow-specific endpoint
    ...
    if [ "$WORKFLOW_RUN_ID" == "null" ] || [ -z "$WORKFLOW_RUN_ID" ]; then
        ...
        echo "${RUN_ID},${i},${LOCAL_START},${LOCAL_END},${LOCAL_DURATION},0,TRIGGER_FAILED,,2x" >> "${SUMMARY_FILE}"
    ...
    # Save metrics with both local and GitHub durations
...
EOF
    ...
    echo "${RUN_ID},${i},${LOCAL_START},${LOCAL_END},${LOCAL_DURATION},${GITHUB_DURATION},${CONCLUSION},${WORKFLOW_RUN_ID},2x" >> "${SUMMARY_FILE}"
    
    if [ "$CONCLUSION" == "success" ]; then
        echo -e "${GREEN}✓ Scaled Run ${i} completed${NC}"
...
SCALED (2x) GITHUB ACTIONS BENCHMARK SUMMARY (FIXED)
=====================================================
